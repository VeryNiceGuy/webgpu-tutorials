class Renderer {
    ...
    async init() {
        ...
        await this.loadIndices("shape.indices");
    ...
    async loadIndices(filename: string) {
        const file = await fetch(filename);
        const text = await file.text();
        const indices = text.split(',').map(item => Number.parseInt(item.trim()));
        ...
        const indexBufferPtr = new Int16Array(this.indexBuffer.getMappedRange());
    ...
    createCommandBuffer(): GPUCommandBuffer {
        ...
        pass.drawIndexed(this.indexBuffer.size / 2);
        pass.end();
